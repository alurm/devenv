...
/// filtering. In particular this means the error returned by eval is always
/// shown, fixing the #2820 symptom where a stale `warning: …` log line was
/// selected as the error and the real eval error was dropped.
...
- To use 'git-hooks', run the following command:
    ...
    fn format_eval_error_renders_bare_syntax_error() {
        // Syntax-error shape from the original #2820 reproduction: a single
        // `error: …` paragraph with source context and no frames.
        let raw =
    ...
    fn format_eval_error_always_includes_the_eval_error() {
        // Regression lock for the root cause of #2820: a stale Nix
        // `warning: …` line must never replace the eval-returned error.
        // The raw FFI text is rendered in full, so even if a warning is
        // present the real error follows it at the bottom.
        let raw = "\
