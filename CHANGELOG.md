...
- Fixed misleading evaluation errors where a stale Nix warning (e.g. `Ignoring the client-specified setting 'system'` for untrusted users) replaced the real error message, hiding syntax errors and suggestions like `devenv inputs add …`. The error returned by evaluation is now always shown ([#2820](https://github.com/cachix/devenv/issues/2820)).
